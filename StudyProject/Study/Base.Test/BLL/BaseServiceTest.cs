...
public class BaseServiceTest
{
    ...
    public BaseServiceTest()
    {
        ...
        _testEntityService = new TestEntityService(
            uow,
            repository,
            new BllDalMapper<TestEntity, TestEntity>(mapper));
    }
    ...
    public async Task Test1()
    {
        ...
        var added = await _testEntityService.AddAsync(new TestEntity() {Value = "TestValue"});
        ...
        // Should be only 1 entity in DB
        Assert.Equal(1, data.Count());

        // Add second entity
        var added2 = await  _testEntityService.AddAsync(new TestEntity() {Value = "Test2"});
        await _ctx.SaveChangesAsync();

        var id2 = added2.Id;

        // Try to remove invalid ID, should return 0 entries deleted

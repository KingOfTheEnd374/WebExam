...
const loadData = async () => {
    ...
    const response = await TaskService.get(authStore.jwt!, id);
    if (response.data) {
        name.value = response.data.taskName;
        sort.value = response.data.taskSort;
        due.value = response.data.dueDt.substring(0, 16);
        completed.value = response.data.isCompleted;
        archived.value = response.data.isArchived;
        categoryId.value = response.data.todoCategoryId;
        priorityId.value = response.data.todoPriorityId;
    }
...
const validateAndCreate = async () => {
    ...
    const response = await TaskService.update(authStore.jwt!, id, name.value, sort.value, (new Date()).toISOString(), due.value, completed.value, archived.value, categoryId.value, priorityId.value, (new Date()).toISOString());
    router.push("/tasks");
};
